...
* **notifications:** the settings page lists the devices subscribed to web push
  and can remove them. A subscription was identified only by its endpoint — a
  push service URL, nothing a person recognises — so the page could offer
  nothing but "this device on / off". Somebody who had enabled notifications on
  a phone they no longer own could not see that, let alone stop it. Each row is
  named from the user agent that subscribed, matched against a fixed list so the
  stored string never reaches the page, with the date, a marker for the browser
  you are looking at, and a remove button. The page is handed a non-reversible
  handle, never the endpoint or the key material.

### Security

* **notifications:** one account could take over another's subscribed device.
  The endpoint is unique across the table, so a second account storing the same
  string moved the row: the first person's phone quietly stopped being notified
  and every notification for that device went elsewhere, with nothing on any
  screen to say so — the settings page reads the browser's subscription, not the
  stored row. The endpoint is not a secret; it travels to the push service on
  every send, sits in the browser's own storage, and is in any database backup.
  The p256dh is not shared, so an endpoint now changes hands only for a request
  that can show it: the shared family browser can (getSubscription() returns the
  existing subscription, keys and all), somebody who merely learned the address
  cannot. Measured before the fix: "Alice 1 → 0, Mallory 0 → 1".

* **notifications:** the VAPID keypair is claimed on first use rather than
  written blindly. It is generated by whichever request arrives first, and two
  at once both found the rows empty, both generated, and two separate writes let
  one overwrite the other between its own halves — measured: public key from one
  pair, private key from the other. A browser that had already subscribed is
  bound to the applicationServerKey it saw, so a changed pair means HTTP 403 on
  every send from then on; 403 is not 404/410, so the subscription is never
  cleaned up either, and it fails every night with nothing on any screen to say
  why. The pair is now written whole inside one transaction and read back, and a
  group left half-written by an older version is reported as unconfigured rather
  than completed with a foreign half.

* **notifications:** the subscription fields are checked for shape and length at
  the door. They were stored as whatever arrived, so one authenticated request
  could write two megabytes into the table and come back with a fresh endpoint
  for the next one. RFC 8291 fixes the shape — a 65-octet P-256 point beginning
  0x04, an auth secret of exactly sixteen — and anything else is a row no
  notification can ever reach.

* **notifications:** a payload past the RFC ceiling is refused instead of being
  sent unreadable. The header announces a record size of 4096 and RFC 8291 §4
  states the boundary outright: at most 3993 octets of plaintext. Past it the
  record overruns what its own header declared, the receiver finds a plaintext
  octet where the padding delimiter should be and must discard it — while the
  push service answers 201 and the log says the notification went out.

* **notifications:** a clicked notification can no longer send the browser
  off-origin. Defence in depth: nothing reachable today puts a foreign URL in a
  payload, because the cron hard-codes the target and RFC 8291 encryption stops
  the push service substituting one. Relative URLs resolve against the service
  worker's own location, so an instance served under a sub-path keeps its own
  root instead of being sent to `/`.

### Changed

* **notifications:** a web push is kept for two days past the renewal it names,
  not for four weeks. The TTL was a fixed 2419200 seconds, which is not a choice
  but the maximum every push service accepts — "hold this as long as you
  possibly can", applied to a message that stops being true in days. A phone
  switched on after a holiday produced a pile of notices about renewals long
  past. RFC 8030 §5.2 makes the header mandatory and lets a service keep a
  message for less than asked, so the cap costs nothing.

* **notifications:** an account keeps at most twenty subscribed devices, oldest
  dropped rather than newest refused. Every row is one HTTP request per message
  in the nightly run, five seconds to connect and fifteen to read — and that run
  also sends the email, Telegram and webhook notifications for the rest of the
  household. Nobody's new phone should fail to subscribe because of four devices
  they threw away.
