...
# Directories the audit does not look at.  The first two are third-party code
# that Wallos does not own — libs is vendored by hand, vendor is installed by
# Composer; the next two are the permitted SQLite implementation boundary from
# issue #41.  They do not exist yet — issue #20 creates them — and listing them
# now means the baseline shrinks by itself as code moves in.
#
...
EXCLUDED_PATHS='libs vendor includes/database migrations/sqlite .claude'
