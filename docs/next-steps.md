...
## The remember-me token fix — shipped in 5.11.1, severity corrected on the way

`restoreSessionFromRememberMeCookie()` now always validates the cookie's token.
It skipped it when `admin.login_disabled` was set. This was first read as an
OIDC-only account takeover and held under embargo; a review, then a hard test
(`remember_me_flag_diagnosis`), **disproved that** — `admin.login_disabled` is
Wallos's single-user no-login mode (where `login.php` already authenticates user
1 with no password, enable-able only with one user and registrations closed),
not the OIDC `oauth_settings.password_login_disabled` toggle, which this
function never read. An OIDC-only installation was never exposed; both our
instances have `admin.login_disabled = 0` regardless. So it was a
defense-in-depth / correctness fix, and it shipped normally in **5.11.1** — no
embargo, no upstream disclosure. We decided not to write to the maintainer.

The follow-up the review pointed at is the real one: `oidc_login.php` mints a
30-day `login_tokens` row and cookie on *every* OIDC sign-in, with no "remember
me" choice, so re-entry for up to 30 days rests on Wallos's own token rather
than the IdP. That is exactly what the milestone-I-style **IdP-authority work
(`oidc/idp-authority`)** takes apart: back-channel logout independent of cookie
expiry, `invalid_grant` on a stale-token refresh terminating the session, and
