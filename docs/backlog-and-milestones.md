...
- **5.16.0** (2026-09-07) — minor. The one-off default-name localizer moved out of settings.php (two blocks ~500 lines apart) onto its own page `localize.php`: both halves, one button, reachable only from the dashboard banner, and it redirects itself away once an account has nothing left to rename. Fixes the banner that stayed up after a user renamed only the currencies — found on the test instance during the 5.15.2/5.15.3 check. Closes the test hole that allowed it (no case could produce a payment-method candidate). Built for deletion: three files; the detection helpers stay because they also seed new accounts in their own language.
- **5.16.1** (2026-09-07) — patch, both found by Thorsten on the test instance right after the 5.16.0 deploy (#172). The migration page's redirect fired after the document had started, so it was discarded and the page stopped after the navigation instead of returning to the dashboard — the whole mechanism by which the page removes itself. Moved to `includes/checkredirect.php`. The per-row checkboxes were invisible (Wallos hides checkboxes and draws a switch via an adjacent label; these rows wrap theirs), so no entry could be excluded. New gate: no page may redirect after including the header — `admin.php` is a recorded known exception, an admin guard wants its own review.
- **5.16.2** (2026-09-07) — patch, release defect. 5.16.1 was tagged with an empty `includes/version.php`, so `$version` was undefined and the cache buster on every asset URL was broken — with `display_errors` off it would have been silently empty and identical for every version, so 5.16.1's CSS fix would never have reached a warm browser cache. Cause: a one-line read-and-write of the file truncated it before reading. Two gates added (version file usable and set when included; version equals the newest changelog entry), and `build` depends on `test`, so this cannot produce an image again.
- **5.16.3** (2026-09-07) — patch. The calendar's "active subscriptions" tile counted payment occurrences instead of subscriptions (five subscriptions, one paying daily, reported as 29 — reported by Thorsten, arithmetic reproduced from the raw rows by the operator session). The projection moved to `includes/calendar_occurrences.php` and returns both counts separately; six cases cover it. The amounts were never wrong. The same loop stands in upstream/main, so this is an upstream defect too and a PR candidate.
- **5.16.4** (2026-09-07) — licence compliance (#171). ApexCharts pinned to 4.7.0, the last MIT release, replacing 5.15.2 which the 5.x licensing change put outside what GPL-3.0 may redistribute; provenance and MIT text recorded in THIRD_PARTY_LICENSES.md, tarball verified against the npm integrity hash. One real 4.x incompatibility found by rendering both versions headless: the donut legend toggle left the removed slice on screen — fixed version-neutrally. Two cases gate the vendored licence, which no behavioural test can catch.
- **5.16.5** (2026-09-07) — patch (#172 follow-up). The localizer page rendered a header and nothing else: the redirect guard added in 5.16.1 loads localization candidates before header.php, and a language file already required from inside wallos_translations() makes header.php's require_once a no-op, so global $i18n never existed and the page died on its first translate(). header.php now uses require. Verified in a container against the released image: 0 candidate rows before, all 37 after.
- **5.16.6** (2026-09-08) — patch (#173). The admin page's non-admin guard redirected after header.php had printed the document, so the Location header was discarded: HTTP 200 with navigation, a warning and nothing else instead of a redirect. Measured against the released image (headers_sent() true at header.php:89, no output buffering; 200/57961 bytes before, 302/0 bytes after, admin still served). No content was ever exposed. The guard moved to checkredirect.php and asks wallos_user_is_admin() directly, because $isAdmin is not assigned until header.php:62. The 5.16.1 ordering gate now has no exceptions.

- **5.17.0** (2026-09-12) — minor. **Upstream 5.7.1 merged**, and eleven of the thirty-three commits were this fork's own pull requests coming home (#1190, #1192–#1200, #1202), so most of the thirty-four conflicts were our fix meeting its upstream shape and were resolved to this tree's version. Genuinely new from upstream: Markdown subscription notes (vendored Parsedown 1.8.0 in safe mode, storage back to plain text, migration 000088 decodes what was stored); a per-user dashboard limit for upcoming payments (migration 000087); upcoming cancellations on the dashboard and in the statistics, with the monthly and yearly amount they would stop costing; `manifest.php` replacing `manifest.json` so an installed PWA's chrome follows the viewer's theme; Azerbaijani plus reworked Korean and a regenerated CLDR dataset; unused uploaded logos removed when a subscription or an account goes. Fork-side: `deleteLogoFileIfUnused()` named one bound parameter three times in one statement, which PDO with native prepares would have refused on PostgreSQL; upstream's `000057`/`000058` run here as `000087`/`000088` through the boundary; the two new dashboard helpers compare against a `gmdate()` boundary bound in PHP instead of SQLite's `date('now')`. Two defects fixed that the merge brought in or exposed: `webhookJsonEscape()` from upstream 5.7.0 destroyed every webhook payload whose note ends in a quotation mark (`trim($encoded, '"')` eats the quote of the value's own `\"` escape), and `createdatabase.php` recreated the `notifications` table on every container start, which made migration 000016's drop correct and useless — it held for exactly one boot. Three of our own gates were wrong and are fixed rather than worked around: two walked `.claude/` agent worktrees, the `ORDER BY` gate read a string concatenation and its own explanatory comment as defects, and `portable_sql()` could only read a query written on one line. 966 tests pass on SQLite and PostgreSQL 14.

## On main, unreleased (next release candidate)
- _Empty_ — everything above shipped in 5.17.0.
