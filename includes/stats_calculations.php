...
if (isset($_GET['category']) && $_GET['category'] !== '') {
    ...
    $noCategoryDefaultName = wallos_default_no_category_name($userData['language'] ?? 'en');
...
if ($result) {
    ...
    if (isset($subscriptions)) {
        ...
        foreach ($subscriptions as $subscription) {
            ...
            // Tallied only where the key was seeded: a NULL payer, a deleted
            // category or a disabled payment method names a group the arrays
            // above never carried, and indexing it anyway sprayed warnings
            // over every stats request (upstream #1179/#1182, fix as in
            // upstream PR #1183). Such a subscription still counts and costs
            // below — it just belongs to no displayable group.
