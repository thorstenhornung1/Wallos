...
function wallos_totp_disable($db, $userId)
{
    ...
    // SQLite3 has no beginTransaction()/commit()/rollBack() — those are PDO's.
    // The transaction is run as statements, which is what the class offers.
    if ($db->exec('BEGIN') === false) {
        error_log('Wallos: could not start a transaction to disable 2FA for user ' . $userId
            . ': ' . $db->lastErrorMsg());
