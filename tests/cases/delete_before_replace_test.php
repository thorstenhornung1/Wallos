...
function delete_before_replace_files()
{
    ...
    foreach ($walk as $file) {
        ...
        // directories hold .git and the agent worktrees, which are whole
        // checkouts of this repository nested inside it. Scanning them reports
        // every finding twice, against paths that do not exist in the tree
        // under review.
        if (wallos_test_repo_excluded($relative) || strpos($relative, 'tests/') === 0) {
            continue;
        }
...
function delete_before_replace_result_is_read($lines, $index)
{
    ...
    // this the window below decides it, and a flag that gates a commit forty
    // lines later reads as a dropped result.
...
wallos_test('the second row really does get in, and reading the result is what stops it', function () {
    // The delete is made to fail with a SQLite trigger body — BEGIN SELECT
    // RAISE(ABORT, ...) END — which PostgreSQL does not have; there a trigger
    // needs a PL/pgSQL function, and writing one would be testing the server
    // rather than the endpoint. The defect and its fix are both in application
    // code, so proving it on one backend proves it.
    if (wallos_test_skip_unless_sqlite('the failure is injected with a SQLite trigger body')) {
        return;
    }
