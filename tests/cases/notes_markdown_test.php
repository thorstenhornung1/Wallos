...
wallos_test('the notes migration decodes existing escaped notes, and is safe to run twice', function () {
    // Upstream numbers this migration 000058; that number was already this
    // fork's user_roles table when it arrived, so it runs here as 000088.
    ...
    // The date is computed in PHP and bound: SQLite's date('now', '+5 days')
    // is not a function PostgreSQL has, and this case is about the notes
    // column rather than about either backend's date arithmetic.
    $stmt = $db->prepare("INSERT INTO subscriptions (user_id, name, price, currency_id, next_payment, cycle, frequency, inactive, notes)
                          VALUES (1, 'Netflix', 9.99, :currencyId, :nextPayment, 3, 1, 0, :notes)");
    ...
    require WALLOS_ROOT . '/migrations/000088.php';
