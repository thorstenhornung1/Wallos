...
/*
  ...
  Upstream 5.6.0 carries this as includes/exchange_rate_freshness.php, which is
  where the fix was sent (#1199). This fork already had the same answer inside
  includes/currency_provider.php, next to the client that acts on it, so the
  merge kept one implementation rather than two that can disagree. The cases
  below are upstream's, pointed at the function this tree actually calls.
*/
...
wallos_test('a refresh from today counts and one from yesterday does not', function () {
    ...
    assert_true(wallos_exchange_rates_fresh($db, 1), 'today is current');
    assert_true(wallos_exchange_rates_fresh($db, 2) === false, 'yesterday is not');
...
wallos_test('an account that never refreshed is not current', function () {
    ...
    assert_true(wallos_exchange_rates_fresh($db, 1) === false,
...
wallos_test('both refresh paths ask before they fetch', function () {
    ...
    foreach ([
        ...
        assert_contains('wallos_exchange_rates_fresh($db, $userId)', $source,
            $path . ' asks whether the rates are already current');
        assert_contains('currency_provider.php', $source,
            $path . ' includes the file that answers it');
    }
