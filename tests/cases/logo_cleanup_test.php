...
wallos_test('deleting an account clears its logos but not another account\'s', function () {
    ...
    // The routine both deletion endpoints call, not a transcription of it.
    // Upstream inlines the gather-delete-sweep into deleteaccount.php and
    // deleteuser.php and its own test replays those statements; here there is
    // one implementation, so the test can ask it directly and a change to it
    // cannot pass by leaving the replay behind.
    require_once WALLOS_ROOT . '/includes/user_deletion.php';

    $deletion = wallos_delete_user($db, 2, $dir);

    assert_true($deletion['success'], 'the account was deleted: ' . (string) $deletion['error']);
