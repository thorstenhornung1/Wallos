...
/*
  ...
  Upstream (#1194) runs the transaction as `exec('BEGIN')`; this fork runs it
  through the database abstraction, which is the same transaction on SQLite and
  a working one on PostgreSQL. The assertions below name the abstraction, and
  the runtime case quotes "user" because PostgreSQL treats the bare word as
  reserved.
*/
...
wallos_test('the enrolment writes are one transaction', function () {
    ...
    assert_contains('$db->beginTransaction()', $source, 'the enrolment opens a transaction');
    assert_contains('$db->commit()', $source, 'and commits it');
    assert_contains('$db->rollBack()', $source, 'and rolls back when a write fails');
});
